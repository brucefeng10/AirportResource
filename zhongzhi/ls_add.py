...
[202,203,204,232,233,231,217,272,218,273,219],[178,179,180,181,182,183,258],[181,182,183],[184,185,186],[184,185,186,187,188,189,259]]
...
with open('type5901-274.txt','r') as f3:
    for line in f3:
        jwtype.append(map(float,line.split(',')))

for i in range(m):
    summ[i][125]=jwtype[i][125]
    summ[i][129]=jwtype[i][129]
    summ[i][132]=jwtype[i][132]
...
near=nearjwhd[0]
hdtype=nearjwhd[1]
is_linshi=nearjwhd[2]

addc=[]
with open('is_normal.txt','r') as f6:
    for line in f6:
        addc.append(map(float,line.split(',')))
...
try:
    ...
    for k in range(n):
        for i in range(m-1):
            if summ[i][k]==1 and addc[i][k]==1:
                for j in range(i+1,m):
                    if addc[j][k]==1 and summ[j][k]==1 and alpha[i][j]==0:
            ...
            if i!=j and ls[i][j]==0:
                for k in range(234,261):#临时机位
                    for l in ls_influence[k-234]:#临时机位影响的机位
                        if addc[i][k]==1 and addc[j][l]==1 and summ[i][k]==1 and summ[j][l]==1:
    ...
    for i in range(m):
        for j in range(m):
            if i!=j and alpha[i][j]==0:
                if summ[i][124]==1 and summ[j][125]==1:
                    mod.addConstr(x[i,124]<=1-x[j,125])
                if summ[i][126]==1 and summ[j][125]==1:
                    mod.addConstr(x[i,126]<=1-x[j,125])
                if summ[i][128]==1 and summ[j][129]==1:
                    mod.addConstr(x[i,128]<=1-x[j,129])
                if summ[i][130]==1 and summ[j][129]==1:
                    mod.addConstr(x[i,130]<=1-x[j,129])
                if summ[i][131]==1 and summ[j][132]==1:
                    mod.addConstr(x[i,131]<=1-x[j,132])
                if summ[i][133]==1 and summ[j][132]==1:
        ...
        for j in range(m):
            if i!=j and alpha[i][j]==0:
                ...
                if summ[i][220]==1 and summ[j][190]==1:
                    mod.addConstr(x[i,220]<=1-x[j,190])
                #针对集合C    
                if summ[i][219]==1 and summ[j][204]==1:
                    mod.addConstr(x[i,219]<=1-x[j,204])
                if summ[i][233]==1 and summ[j][204]==1:
                    mod.addConstr(x[i,233]<=1-x[j,204])
                #针对集合B，左/右/中    
                for k in range(206,219):
                    if addc[i][k]==1 and addc[j][k-15]==1 and summ[i][k]==1 and summ[j][k-15]==1:
                        mod.addConstr(x[i,k]<=1-x[j,k-15])
                for k in range(261,274):
                    if addc[i][k]==1 and addc[j][k-70]==1 and summ[i][k]==1 and summ[j][k-70]==1:
                        mod.addConstr(x[i,k]<=1-x[j,k-70])
                for k in range(221,234):
                    if addc[i][k]==1 and addc[j][k-30]==1 and summ[i][k]==1 and summ[j][k-30]==1:
                        mod.addConstr(x[i,k]<=1-x[j,k-30])
                for k in range(220,233):
                    if addc[i][k]==1 and addc[j][k-29]==1 and summ[i][k]==1 and summ[j][k-29]==1:
                        mod.addConstr(x[i,k]<=1-x[j,k-29])
    ...
    for i in range(m):
        for j in range(m):
            if i!=j and alpha[i][j]==0:
                ...
                for k in range(206,219):
                    if addc[i][k]==1 and addc[j][k+14]==1 and summ[i][k]==1 and summ[j][k+14]==1:
                        mod.addConstr(x[i,k]<=1-x[j,k+14])
                #B集合中左机位停了，右机位就不能停，反之亦然
                for k in range(206,219):
                    if addc[i][k]==1 and addc[j][k+55]==1 and summ[i][k]==1 and summ[j][k+55]==1:
